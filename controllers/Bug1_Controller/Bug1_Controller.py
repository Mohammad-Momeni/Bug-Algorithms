...
while not visitedOnce == 2:
    ...
    yValues.append(-y)
    x, y, _ = gps.getValues()
    if abs(x - initialLocation[0]) < 0.05 and abs(y - initialLocation[1]) < 0.05:
        visitedStart = True
    if (abs(x - finalDestination[0]) < 0.1 and abs(y - finalDestination[1]) < 0.1) and visitedStart:
        visitedStart = False
        visitedOnce += 1
